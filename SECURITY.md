...
There is no bug bounty, but credible reports get prompt fixes and credit in release notes if desired.
...
| JWTs | Theft / replay | 7-day default expiry (`JWT_EXPIRATION_HOURS`, see `backend/settings.py`), `Authorization: Bearer` only, HTTPS at the edge. Tokens are held in `localStorage`, so a successful XSS would expose them — shortening the TTL and moving to an httpOnly cookie is the next hardening step |
...
- **LLM prompt injection.** Retrieved documents are user-controlled by design. We don't claim safety against an attacker who uploads an adversarial PDF that redirects LLM output — the system prompt constrains the model to the supplied context and to cite sources, but that is a mitigation, not a guarantee. Sandboxed system prompts and citation auditing are not implemented.
...
- [x] Rate limits per-route (auth 10/min, chat 30/min, upload 10/min, default 200/hour) via Flask-Limiter, keyed on user id when authenticated and IP otherwise. Storage defaults to `memory://`, which is per-process — set `RATELIMIT_STORAGE_URI` to Redis before running more than one gunicorn worker
- [x] Upload hardening: extension allow-list, magic-byte sniffing, per-user quota, per-file size cap ([backend/security.py](backend/security.py))
- [ ] Malware scanning — `scan_for_malware()` is a deliberate no-op hook; wiring ClamAV needs a `clamd` daemon in the compose stack
